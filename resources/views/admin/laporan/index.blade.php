                      ...
                      <th scope="col">No</th>
                                    ...
                                    @foreach($tracking as $item)
                                        ...
                                        <tr>
                                            ...
                                            <td><center>{{$item->rw->nama_rw}}</center></td>
                                            <td><center>{{$item->positif}}</center></td>
                                            <td><center>{{$item->sembuh}}</center></td>
                                            <td><center>{{$item->meninggal}}</center></td>
                                            <td><center>{{$item->tanggal}}</center></td>
                                            <td>
                                            <form action="{{route('tracking.destroy',$item->id)}}"  method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <center><a href="{{route('tracking.show',$item->id)}}" class="btn btn-secondary btn-sm"><i class="fa fa-eye"></a></i>
                                            <a href="{{route('tracking.edit',$item->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></a></i>
